...
## USAGE   'wae *.mp3 path/file.mp4 16 256' plays sound files and waits from 16 to 256 seconds between each play.

## Multiple Termux sessions can be used in order to run commands 'wae arguments' simultaneously and to switch seamlessly between plays.
...
[ "${1:-}" != "" ] && { { [[ "${1//-}" = [Hh] ]] || [[ "${1//-}" = [Hh][Ee] ]] || [[ "${1//-}" = [Hh][Ee][Ll] ]] || [[ "${1//-}" = [Hh][Ee][Ll][Pp] ]] ; } && { TMPCMD="$(sed -n '2,12p' $0 | sed 's/##\ //g')" && printf '\e[0;32m%s\e[0m\n' "${ARG0^^} HELP $TMPCMD" ; exit ; } ; }
for TMPVRBL in "$@"
do
